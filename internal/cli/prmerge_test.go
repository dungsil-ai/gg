...
func TestTranslatePRMerge(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "glab merge 방식",
			...
			want: Invocation{Bin: "glab", Args: []string{"mr", "merge", "42", "--merge", "--when-pipeline-succeeds=false", "--repo", "https://gitlab.com/o/r"}}},
		{name: "glab rebase 방식",
			req: Request{Resource: "pr", Action: "merge", Number: "42", Rebase: true}, repo: gl, p: GLab,
			want: Invocation{Bin: "glab", Args: []string{"mr", "merge", "42", "--rebase", "--when-pipeline-succeeds=false", "--repo", "https://gitlab.com/o/r"}}},
		{name: "glab squash 방식",
