...
func TestTranslateRelease(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "gh release download 빈 형태는 전체 자산 glob",
			req:  Request{Resource: "release", Action: "download"},
			repo: gh, p: GH,
			want: Invocation{Bin: "gh", Args: []string{"release", "download", "--pattern", "*", "-R", "github.com/o/r"}}},
