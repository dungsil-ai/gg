...
func resolvePlan(req Request) (executionPlan, error) {
	...
	// delete, label edit·delete, issue 관계 등록, issue comment list/edit/
	// delete, issue edit, issue delete는 provider를 고른 뒤 미지원을 확정하므로
	// tea login을 묻지 않는다. label list·create는 tea도 중계하므로 login을 묻는다.
