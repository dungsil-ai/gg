...
- `issue`
  ...
  - [x] `gh issue pin` (대응: `gg issue pin`; glab·tea에 이슈 고정 하위 명령이 없어 미지원)
  - [x] `gh issue reopen` (대응: `gg issue reopen`)
  - [x] `gh issue status` (대응: `gg issue status`; glab·tea에 같은 역할의 이슈 현황 하위 명령이 없어 미지원)
  - [x] `gh issue transfer` (대응: `gg issue transfer <number> <destination-repository>`; glab·tea에 이슈 이동 하위 명령이 없어 미지원)
...
- `issue`
  ...
  - [ ] `glab issue todo` (glab에 issue todo 하위 명령이 없어 중계 대상이 아님 — 문서 기준)
...
- `mr`
  ...
  - [x] `glab mr todo` (대응: `gg pr todo`; gh·tea에 To-Do 추가 하위 명령이 없어 미지원)
...
- `히스토리 재작성`
  ...
  - 안전장치: 작업 트리가 깨끗해야 하고 stash가 비어 있어야 하며 detached HEAD가 아니고 연결 worktree도 없어야 합니다. `--force` 없이 재작성하지 않습니다. 재작성 전 `<git dir>/filter-repo-backup`에 mirror 백업을 만들고(이미 백업이 있으면 사용자가 확인한 뒤 직접 치워야 함), 재작성 대상은 branch·tag와 원격 추적 ref(refs/remotes)입니다 — 원격 추적 ref를 남겨 두면 예전 커밋이 gc에서 살아남아 비밀 제거가 불완전해집니다. 완료 후 재작성된 ref의 force-push를 안내합니다(직접 실행하지는 않음). `--repo`·`--remote`·`--explain`은 지원하지 않습니다.
- `저장소 문맥`
